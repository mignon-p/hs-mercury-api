...
my $paramTestFile = "$rootDir/tests/param-test.hs";
...
sub emitParamHeader {
    ...
    emit "import Data.Text (Text)";
    emit "import qualified Data.Text as T ( unpack, pack )";
...
sub emitParamTestHeader {
    emit "-- Automatically generated by util/generate-tmr-hsc.pl";
    emit '{-# LANGUAGE OverloadedStrings #-}';
    emit '';
    emit 'import Test.HUnit ( Test(..), runTestTT, assertEqual )';
    emit '';
    emit 'import qualified System.Hardware.MercuryApi as TMR';
    emit '';
    emit 'main = runTestTT tests';
    emit '';
}
...
sub emitParamTests {
    ...
    foreach my $param (@params) {
        ...
        my $camel = makeCamel($path);
        if ($camel ne "") {
            my $testname = "test$camel";
            push @testNames, $testname;
            emit "$testname :: Test";
            emit "$testname = TestCase \$ do";
            emit "  assertEqual \"TMR.paramName TMR.$param\"";
            emit "    \"$path\"";
            emit "    (TMR.paramName TMR.$param)";
            emit "  assertEqual \"TMR.paramID \\\"$path\\\"\"";
            emit "    TMR.$param";
            emit "    (TMR.paramID \"$path\")";
            emit "";
        }
    }
