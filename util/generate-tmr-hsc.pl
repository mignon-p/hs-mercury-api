    ...
    while (<F>) {
        if (m%^\s*/\*\*\s*(.*?)\s*\*+/%) {
            ...
            unless ($1 eq "PARAM_MIN" or
                ...
                my $p = $1;
                push @params, $p;
                my ($t, $c, $path) = parseParamComment ($comment);
                $params{$p} = $c;
                $paramType{$p} = $t;
                $paramPath{$p} = $path;
            }
...
sub emitParamHeader {
    ...
    emit "    -- | These functions are automatically generated, so not";
    emit "    -- all of them make sense.  For example,";
    emit "    -- 'paramSetRadioTemperature' will always throw";
    emit "    -- 'MercuryException' with status 'ERROR_READONLY'.";
    my $sep = " ";
